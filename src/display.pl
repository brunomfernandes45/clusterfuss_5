...
display_column_positions_aux(Size, Acc) :- Acc1 is Acc + 1, write(Acc1), write('   '), display_column_positions_aux(Size, Acc1).
...
display_cells(Size, Row, P) :- Code is 0x41 + P, put_code(Code), write(' '), put_code(0x2502), display_cells_aux(Size, Row).
